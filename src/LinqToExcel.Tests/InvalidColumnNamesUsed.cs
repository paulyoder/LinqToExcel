...
namespace LinqToExcel.Tests
{
    ...
    public class InvalidColumnNamesUsed
    {
        ...
        {
            Assert.That(() => (from x in ExcelQueryFactory.Worksheet("Sheet1", _excelFileName)
        ...
        {
            Assert.That(() => (from x in ExcelQueryFactory.Worksheet<CompanyWithCity>("Sheet1", _excelFileName)
                               where x.City == "Omaha"
                               select x).ToList(),
